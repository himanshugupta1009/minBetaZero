...
include("custom_layers.jl")

global const DEBUG :: Base.RefValue{Bool} = Ref{Bool}(false)
...
function (ac::ActorCritic)(x; logits=false)
    ...
    value_input, policy_input = _head_inputs(encoded_input)
    value = ac.critic(value_input)
    policy = ac.actor(policy_input; logits)
    return (; value, policy)
end
...
function getloss(ac::ActorCritic, input; value_target, policy_target)
    ...
    value_input, policy_input = _head_inputs(encoded_input)
    value_loss, value_mse = getloss(ac.critic, value_input; value_target)
    policy_loss = getloss(ac.actor, policy_input; policy_target)
    return (; value_loss, value_mse, policy_loss)
end

_head_inputs(encoded_input) = (encoded_input, encoded_input)
_head_inputs(encoded_input::Tuple) = (encoded_input[1], encoded_input[2])
